...
function V2MarkdownContent({ content }: { content: string }) {
  const toId = (children: unknown): string => {
    ...
    return t.toLowerCase().replace(/[^a-z0-9_\- ]/g, "").replace(/ /g, "-");
  };
  ...
  const components = {
    ...
    h4: ({ children }: any) => <h4 id={toId(children)} style={{ ...headingBase, fontSize: "0.95rem" }}>{children}</h4>,
    h5: ({ children }: any) => <h5 id={toId(children)} style={{ ...headingBase, fontSize: "0.9rem" }}>{children}</h5>,
