...
import { EditableText, SegmentImage } from "./components/reportPrimitives";
import { ReportMiniMap } from "./components/ReportMiniMap";
import {
  renderBandBadge, renderBandDonut, renderTopRiskFullPage, renderTopRiskGrid,
  renderTopRiskTabular, renderTreatmentSummary,
} from "./components/reportSectionRenderers";
import type {
  TopRiskRenderContext, TreatmentSummaryRenderContext,
} from "./components/reportSectionRenderers";
...
export default function ReportBuilderPage() {
  ...
  // ── Network (all-profile) project list for Benchmarking Stats comparison ──
  // allProfileProjects: every project in the active profile (fetched on mount).
  // The network score rows themselves are fetched lazily inside useReportData.
  ...
  // ── Server data spine (see hooks/useReportData.ts) ────────────────────────
  // Whether any Benchmarking Stats section is visible — gates the lazy network
  // fetch inside the hook (same condition the inline effect used).
  const benchmarkVisible = elements.some((e) => e.type === "benchmarkStats" && e.visible);
  const {
    fullDataset, filteredDataset, networkDataset,
  ...
  // named active filters (see the hook for the filtered-row derivation).
  ...
  // ── Section renderer contexts (see components/reportSectionRenderers.tsx) ──
  // Plain object literals (recreated per render, matching the closures they
  // replaced); the render functions consume them synchronously during render.
  const topRiskCtx: TopRiskRenderContext = {
    getEnriched, getSegmentTreatments, dispNameWithQuarter,
    handleUploadTreatmentImageClick, secTitle, setSecTitle,
  };
  const treatmentSummaryCtx: TreatmentSummaryRenderContext = {
    dispName, setProjectName, projectQuarterLabel,
  };
          ...
          return (
            <div style={{ height: "100%", display: "flex", flexDirection: "column", overflow: "hidden" }}>
              {displayRows.length === 0 ? (
                ...
                renderTopRiskFullPage(topRiskCtx, displayRows, el.id)
              )}
            </div>
          );
        }
        ...
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {displayRows.length === 0
              ...
              : viewMode === "grid" ? renderTopRiskGrid(topRiskCtx, displayRows)
                : renderTopRiskTabular(topRiskCtx, displayRows)}
          </div>
        );
      }
            ...
            {chunks.map((chunk, i) => {
              ...
              return (
                <div key={i} style={{ height, boxSizing: "border-box", paddingBottom: isLast ? 0 : PAGE_GAP, flexShrink: 0, display: "flex", flexDirection: "column", overflow: "hidden" }}>
                  ...
                  <div style={{ flex: 1, overflow: "hidden" }}>
                    {renderTreatmentSummary(treatmentSummaryCtx, chunk, projectSegmentCounts)}
                  </div>
                </div>
              );
            })}
