"""Curvature analysis extracted from the GIS spatial-query class.

Owns the path-network geometry handling and the minimum-radius / angle-based
curvature detection used by CycleRAP autocoding and the curvature visualization
route. Instances hold a back-reference to the parent :class:`GIS`
(``self.gis``) and reach shared map data through ``self.gis.store``.

Public entry points (also exposed on GIS as thin delegators):
  - get_curvature(point, ...) -> (category, subcategory)
  - analyze_curvature(point, ...) -> dict of radius/junction diagnostics
  - get_curvature_visualization(point, ...) -> dict for the map overlay
  - get_radius_and_width_at_point(point, ...) -> (min_radius, width[, details])

See REFACTOR_PLAN.md S3.4 and CLAUDE.md for why the four monkeypatched methods
are reached back through ``self.gis`` inside this module.
"""
...
class CurvatureAnalyzer:
    """Path-network curvature detection for a parent :class:`GIS` instance."""

    def __init__(self, gis):
        # Back-reference to the owning GIS; shared map data is read via
        # self.gis.store (see the module docstring / REFACTOR_PLAN S3.4).
        self.gis = gis
            ...
            for layer_key in priority:
                try:
                    ...
                    if found_width is None:
                        ...
                        intersecting_std = self.gis._standardize_width_column(intersecting)
    ...
    def _load_path_layer(self, store_key):
        raw_gdf = self.gis.store.get(store_key)
    ...
    def _flatten_line_geometries(geom):
        ...
        if geom.geom_type == "GeometryCollection":
            ...
            for part in geom.geoms:
                lines.extend(CurvatureAnalyzer._flatten_line_geometries(part))
            return lines
        return []
    ...
    def analyze_curvature(self, point, sharp_turn_threshold=10.0, default_value=2, collect_radius=5.0, include_diagnostics=False):
        input_point = self.gis.store.to_metric_point(point)
        analysis_point, snap_info = self.gis._snap_point_to_path_network(point)

        min_radius, width, analysis_details = self.gis.get_radius_and_width_at_point(
        ...
        if is_sharp_curve:
            ...
            has_kink, has_path_junction = self.gis._check_angle_curvature(analysis_point, collect_radius=collect_radius)
    ...
    def get_curvature(self, point, sharp_turn_threshold=10.0, default_value=2):
        ...
        """
        analysis = self.gis.analyze_curvature(
            point,
            sharp_turn_threshold=sharp_turn_threshold,
