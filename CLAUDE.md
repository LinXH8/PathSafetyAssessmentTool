...
### Report Builder: Huge "Black Watermark" / Gradient on PDF Export with Overall Sections (2026-07-20)

**Symptom:** When toggling on "Include overall sections" and downloading a PDF, the resulting PDF contains a massive black gradient or shadow overlay covering the right side of the page (which looks like a large "black watermark").

**Root cause — html2canvas box-shadow rendering bug on large canvases.** In the Report Builder UI, each "A4 page" preview has a `box-shadow` applied to make it look like floating paper. `html2canvas` tries to render this CSS `box-shadow`. When "Include overall sections" is enabled, the report height roughly doubles. `html2canvas` has a known bug where calculating the blur radius for `box-shadow` on extremely tall canvases overflows or loses precision, resulting in a giant, smeared black gradient across the captured image.

**Fix:** Since the downloaded PDF is the final document and does not need "floating paper shadows", the fix is to strip the shadows during the `html2canvas` capture phase.
1. Added a `rb-page-bg` class to the page background `div`s in `ReportBuilderLayoutV1.tsx` and `ReportBuilderLayoutV2.tsx`.
2. In `usePdfExport.ts`, iterate over `.rb-page-bg` elements in the cloned document and temporarily set their `box-shadow` to `none` before capture, restoring it afterward (just as we do to hide page labels).

**Key files:**
- `frontend/src/pages/ReportBuilderPage/hooks/usePdfExport.ts` — stripped `box-shadow` during `onclone`.
- `frontend/src/pages/ReportBuilderPage/layouts/ReportBuilderLayoutV1.tsx` & `V2.tsx` — added `.rb-page-bg` class.
...
### Auto-code by Attribute: Child/Subtype Fields Silently Dropped (2026-07-20)

**Symptom:** Running "Auto-code (By Attribute)" with only a parent attribute selected (e.g.
"Crossing facility") computed the correct value for its child/subtype attribute (e.g.
"Crossing Type") internally, but the child value never got saved, and never appeared when
clicking the pencil icon next to the parent row to review/edit the subtype in
`AttributeOptionsDialog`.

**Root cause — the same "companion field" allow-list bug existed independently in TWO
places, backend and frontend, both requiring an explicit per-pair entry that was missing:**

1. **Backend** — `backend/app/api/projects/autocode.py`, `_bulk_gen()`. CV
   (`prediction.py::_assign_attributes`) and GIS (`_gis_autocode_core`'s `_needs(parent,
   child)` OR-guards) both compute parent + child together unconditionally. But right before
   saving, an `actual_filter` allow-list keeps only the fields the user explicitly selected
   (`fields_filter`) — so an unrequested child key gets filtered out of `merged` and is never
   written to `ver.attributes.df`. Three pairs already had a companion-append line (Grade→
   Gradient %, Delineation→Delineation Type, Major Surface Deformation or Drain Opening→
   Defect Type); Crossing Facility→Crossing Type did not, and neither did three other
   pairs that were unconditionally computed the same way (Fixed Obstacle on Facility→FO
   Type, Non-Fixed Obstacle on Facility→NFO Type, Curvature→Curvature Sub-category,
   Facility Width per Direction→Facility Width Sub-category).
2. **Frontend** — `frontend/src/pages/CodingPage/hooks/useAutocode.ts`. Independently of the
   backend, the `psat:autocode:by-field` handler does its own selective patch of the
   streamed response into React `attrs` state (so an in-flight by-attribute run doesn't
   clobber unsaved edits to other attributes) — this patch had its own hardcoded companion
   list, and `updateAutocodeBaseline` had a second, separate copy of the same list for
   baseline/"changed" diffing. Both copies only covered the original 3 pairs, so even after
   the backend fix correctly returned Crossing Type in the response, the frontend threw it
   away before it reached `attrs[currentIndex]` — which is what `AttributeOptionsDialog`
   reads via `onEditOptions` in `CodingLayoutV1.tsx`/`CodingLayoutV2.tsx`. This is why the
   pencil-icon modal kept showing nothing even after the backend-only fix.

**Fix:**

- Backend: added the 5 missing `if "<parent>" in actual_filter: actual_filter.append("<child>")`
  lines to the `actual_filter` block in `_bulk_gen()` (~line 585-605).
- Frontend: extracted a single `COMPANION_FIELD_MAP` constant (parent → child field name(s))
  at the top of `useAutocode.ts`, containing all 8 pairs, and replaced both hardcoded
  if-chains (`updateAutocodeBaseline` and the `psat:autocode:by-field` `patchedAttrs`
  builder) with a loop over `COMPANION_FIELD_MAP[field] ?? []`. One list instead of two
  prevents them drifting out of sync with each other or with the backend again.

**Gotcha:** None of these child/subtype fields are independently selectable in the UI
(`frontend/src/constants/autocodeAttributes.ts` `GROUP_RULES`/`KEY_ALIASES` has no entry for
any of them) — they only ever arrive by riding along with their parent. Any *new*
parent/child pair added in the future needs its companion-append line added in **both**
places (`autocode.py`'s `actual_filter` block AND `useAutocode.ts`'s `COMPANION_FIELD_MAP`)
or it will silently reproduce this exact bug.

**Key files:**

- `backend/app/api/projects/autocode.py` — `_bulk_gen()`: `actual_filter` companion-append
  block
- `frontend/src/pages/CodingPage/hooks/useAutocode.ts` — `COMPANION_FIELD_MAP` constant;
  used in `updateAutocodeBaseline` and the `psat:autocode:by-field` handler's `patchedAttrs`
  builder

### Create Project: Map Mode vs Folder Mode "Segments" Count Mismatch (2026-07-21)

**Symptom:** In the Create Project page, the same road showed wildly different "Segments"
counts depending on how it was found — e.g. "LORONG 1 TOA PAYOH" showed **12** in Map mode's
"Roads Found" table (drawn boundary) but **333** in Folder mode's "Search by Folder Name"
table, for what the user expected to be the same underlying road.

**Root cause — the two counts were computed by unrelated pipelines counting different
datasets, not a simple off-by-N bug:**

- **Folder mode** (`_build_source_folder_summary()`, `backend/app/api/projects/source_folders.py:364-426`)
  counts **real photo-derived segments**: it reads EXIF GPS points from every geotagged photo
  in the folder (`get_image_folder_geo`), resamples them every 10 m
  (`cycleRAP_VA.get_geo_points_by_distance`), and counts the resulting line segments for the
  **whole folder**, unclipped. This is the exact same call
  (`_build_project_geo_data_from_points`, unclipped) that **project creation** itself uses for
  Folder mode (`crud.py:729` → `build_project_geo_data(src_dir, None)`) — so this number was
  always correct; it equals what actually gets created.
- **Map mode** (`roads_in_polygon()`, `backend/app/api/projects/gis_queries.py:65-248`) counted
  something entirely different: how many points/features in the **static GIS reference
  shapefile/CSV** (`road_reference.csv` / `ROADSECTIONLINE.shp` — a pre-surveyed cartographic
  layer, not photos) intersect the drawn polygon. This number has no relationship to the road's
  actual survey photos.

Crucially, Map mode's **actual "Create" click already did the right thing** — the frontend
sends the drawn polygon (`selection_geometry`) to `POST /api/projects/folders`, and
`crud.py:729` calls `build_project_geo_data(src_dir, selection_polygon)`, which correctly clips
real EXIF-derived points to the polygon before resampling. Only the **preview** number in the
"Roads Found" table was wrong — it counted an unrelated static dataset instead of previewing
what the same create pipeline would actually produce.

**Fix:** Made Map mode's preview compute a real, polygon-clipped segment count using the same
pipeline creation already uses, instead of the shapefile/CSV intersection count.

1. **New in-process geo-points cache** (`source_folders.py`: `_GEO_POINTS_CACHE` +
   `_get_cached_geo_points()`) — caches each folder's raw EXIF `{latitude, longitude,
   filename}` points, keyed by the same stat-only content hash already used for folder
   summaries (`_build_source_folder_cache_key`). This is deliberately **not** keyed by polygon
   — the cache only avoids re-parsing EXIF from disk on every redraw; the actual
   point-in-polygon clipping and 10 m resampling still run fresh against whatever polygon is
   currently drawn, on every request. A new/different polygon can never be a cache miss in the
   sense of returning stale results — only a changed folder (new/removed photos, detected via
   the existing content hash) invalidates it.
2. **`roads_in_polygon()`** (`gis_queries.py`, inside the `for folder in
   _select_latest_quarter_folders(folders):` loop) — for every **downloaded** road folder
   (`exists: True`), replaced the static `points` value with a real count: fetch the folder's
   cached geo points, clip them to the drawn polygon via
   `_build_project_geo_data_from_points(geo_points, folder, selection_polygon=poly)`, and use
   `len(segments_gdf)`. Wrapped per-folder in try/except so one folder's failure can't break the
   whole "Roads Found" list (falls back to the old shapefile/CSV estimate).
3. **Roads not yet downloaded** (`exists: False`) are intentionally left unchanged — there are
   no local photos to compute a real count from yet, so the static shapefile/CSV number remains
   the only available signal.

**Key files:**

- `backend/app/api/projects/source_folders.py` — `_GEO_POINTS_CACHE` /
  `_get_cached_geo_points()`
...
### Report Builder: Top Risk Stretches Overlap in Native Print (2026-07-20)

**Symptom:** When viewing the Report Builder canvas or printing natively (Cmd+P), there was a visual overlap at the end of the Top Risk Stretches section where it met the Treatment Summary section.

**Root causes:**
1. **Missing bottom gap:** For the final stretch only, the `paddingBottom` was erroneously set to `0` instead of `PAGE_GAP` (24px). The white `.rb-page-bg` elements are sized at `PAGE_H - PAGE_GAP`. Since the final stretch had no padding, its internal content box height became exactly `PAGE_H`, which hung 24px off the bottom of the white page background into the interstitial gap. Because the subsequent section ("Treatment Summary") begins exactly at the top of the next page, this 24px overhang caused a visual collision.
2. **Native print layout engine:** When natively printing, browsers frequently strip `overflow: hidden` from flex containers to avoid data loss. Since `SegmentImage` containers didn't restrict intrinsic dimensions rigidly in print mode, they were allowed to expand, breaking the layout.

**Fix:**
1. Changed `paddingBottom: isLast ? 0 : PAGE_GAP` to consistently apply `PAGE_GAP` for all stretches in `renderTopRiskFullPage`, ensuring the internal content box correctly respects the physical dimensions of the `.rb-page-bg`.
2. Appended a comprehensive `@media print` block to `reportBuilderPage.css` to disable fixed heights, remove manual spacer margins, and hide the simulated paper backgrounds, thereby allowing the browser's native pagination engine to manage page breaks naturally without forcing overlaps.

**Key files:**
- `frontend/src/pages/ReportBuilderPage/components/reportSectionRenderers.tsx` — Fixed `paddingBottom` logic.
- `frontend/src/pages/ReportBuilderPage/reportBuilderPage.css` — Added `@media print` rules.
