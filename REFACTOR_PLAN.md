...
- 2026-07-03 · S1.6 · created `constants/sessionKeys.ts` (`SESSION_KEYS` + `LOCAL_KEYS` + `gisLayerToggleKey()` + `CODING_FILTER_CONTEXT_KEY` re-export) and `hooks/useSessionState.ts` (typed JSON-backed `useState`). Migrated raw storage strings across 8 files: `pathAnalysisPage.tsx` (removed `SESSION_KEY_PREFIX`/`loadState`; `activeFilters`+`hiddenProjects` now use `useSessionState`), `PathAnalysisMapView.tsx` (removed local `VIEWPORT_KEY`; all `pathAnalysisMap_*`/`pathAnalysis_*`/`treatment_*` strings → constants), `reportBuilderPage.tsx` (removed local `LAYOUT_KEY`; 6 session strings → constants), `treatmentDetailPage.tsx` (4 strings → constants), `Sidebar.tsx` (3 strings → constants), `codingPage.tsx` + `GeoDataPanel.tsx` (CODING_FILTER_CONTEXT_KEY now from sessionKeys; gisLayerToggleKey helper). `api/projects.ts` re-exports `CODING_FILTER_CONTEXT_KEY` for barrel compat. TSC clean; lint 374; build 37 (all pre-existing). Key strings are frozen — only definition site changed.
