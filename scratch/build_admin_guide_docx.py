"""
Generates ADMIN_GUIDE_V2(UPDATED).docx from the six admin-guide markdown files
served by the PSAT Help page.
"""
...
DOCS_DIR = Path(__file__).parent.parent / "frontend" / "public" / "docs" / "admin"
OUTPUT   = Path(__file__).parent.parent / "ADMIN_GUIDE_V2(UPDATED).docx"
...
MD_FILES = [
    ...
    "admin-cyclerap-algorithm.md",
    "admin-user-accounts.md",
]
...
def apply_inline_formatting(run_parent, raw: str):
    """
    Parse inline **bold**, `code`, and plain text segments and add runs to
    run_parent (a Paragraph).  URLs / links are rendered as plain text.
    """
...
def parse_md(doc: Document, md_text: str):
    ...
    # flush code / table if file ended mid-block
...
def build_doc():
    ...
    title_run = title_para.add_run("PSAT Admin Guide")
    title_run.bold = True
    ...
    sub_run = sub_para.add_run("CycleRAP v2.11  ·  Version 2  (Updated June 2026)")
